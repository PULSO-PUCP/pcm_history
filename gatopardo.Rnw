...
Figure~\ref{fig:km_epochs} provides a non-parametric representation of continuity conditions through Kaplan--Meier survival functions. The survival curves reinforce the descriptive evidence presented in Table~\ref{tab:epoch_descriptives}. State Formation and the Oligarchic State consistently exhibit the lowest survival probabilities, while the Power-Sharing State and the Transition and Democratic Crisis period exhibit the highest. Democracy without Parties again occupies an intermediate position.
...
@
\caption{Kaplan--Meier survival functions for ministerial duration across institutional epochs. Survival probabilities are estimated from ministerial appointments in the Presidency of the Council of Ministers (PCM) and the Ministry of Economy and Finance (MEF).}
...
km_epoch_port <- survfit(
  Surv(duration, event) ~ epoch + cartera,
  data = datos_pcm_mef
)

s <- surv_summary(km_epoch_port)

s <- s |>
  tidyr::separate(
    strata,
    into = c("epoch","cartera"),
    sep = ", "
  ) |>
  mutate(
    epoch   = gsub("^epoch=", "", epoch),
    cartera = gsub("^cartera=", "", cartera)
  )
...
ggplot(
  ...
  theme_minimal() + scale_color_brewer(type = 'div') + theme(
    legend.position = c(.75, .75),
    legend.background = element_rect(
      fill = "white",
      colour = "grey80"
...
Several observations emerge from these patterns. First, continuity conditions were clearly not constant across institutional epochs. Ministers faced substantially different probabilities of remaining in office depending on the historical context in which they served. Second, the ordering of the epochs does not suggest a simple progression toward increasingly stable ministerial tenure. Instead, continuity appears concentrated in specific historical configurations rather than distributed monotonically across time. Third, some of the strongest continuity conditions emerge during periods commonly associated with restricted political competition or concentrated executive authority, a finding that will become important in the interpretation of the subsequent analyses.

Taken together, the descriptive evidence establishes that continuity conditions varied substantially across institutional epochs. This finding provides the empirical baseline for evaluating whether those differences reflect broader patterns of institutional learning and whether continuity was distributed unevenly across ministerial portfolios.
...
%   data = gatopardo |> filter(cartera %in% c("PCM","MEF"))
% )
% 
% plot(
%  km_epoch,
...
% legend(
%  "topright",
%  legend = levels(as.factor(gatopardo$epoch)),
%  col = 1:5,
%  lwd = 2,
%  bty = "n"
% )
...
% <<>>=
% gatopardo |>
%   filter(epoch=="Democracy without Parties") |>
%   group_by(cartera) |>
%   summarise(
%     N=n(),
%     median_duration=median(duration,na.rm=TRUE),
...
% <<>>=
% coxph(
%   Surv(duration, event) ~ relevel(factor(cartera), ref = "MEF"),
%   data = gatopardo |>
%     filter(epoch == "Democracy without Parties")
% ) |> summary()
% @
...
\subsubsection{Descriptive Patterns of Ministerial Duration}

\subsubsection{Survival Functions Within Institutional Epochs}

\subsubsection{Interpreting Continuity Conditions}

\subsection{Institutional Learning Between Epochs}

\subsubsection{Comparing Survival Across Institutional Epochs}

\subsubsection{Log-Rank Tests of Epoch Differences}
